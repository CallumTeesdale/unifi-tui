...
pub fn render_clients(f: &mut Frame, app: &App, area: Rect) {
    ...
    let clients: Vec<Row> = app.state.filtered_clients
        .iter()
        .map(|client| {
    ...
    let title = match &app.state.selected_site {
        Some(site) => format!("Clients - {} [{}]", site.site_name, app.state.filtered_clients.len()),
    ...
    let help_text = vec![Line::from("↑/↓: Select | Enter: Details | s: Sort | /: Search")];
